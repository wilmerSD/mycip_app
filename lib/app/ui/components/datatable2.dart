...
enum ColumnSize { XS, S, M, L, XL, XXL, XXS, XXXS }
...
class DataTable2 extends DataTable {
  ...
  static const double _headingRowHeight = 46.0;
  ...
  final double xsRatio;
  final double xlRatio;
  final double xxlRatio;
  final double xxsRatio;
  final double xxxsRatio;
    ...
    final widths = List<double>.generate(columns.length, (i) {
      ...
      if (column is DataColumn2) {
        if (column.fixedWidth != null) {
          w = column.fixedWidth!;
        } else if (column.size == ColumnSize.XS) {
          w *= xsRatio;
        } else if (column.size == ColumnSize.S) {
          w *= smRatio;
        } else if (column.size == ColumnSize.L) {
          w *= lmRatio;
        } else if (column.size == ColumnSize.XL) {
          w *= xlRatio;
        } else if (column.size == ColumnSize.XXL) {
          w *= xxlRatio;
        } else if (column.size == ColumnSize.XXS) {
          w *= xxsRatio;
        } else if (column.size == ColumnSize.XXXS) {
          w *= xxxsRatio;
        }
      }
//xxsminRatio
